<template>
  <div class="box" style="">
    <SelectCountry :item-current="countryCurrentId"
                   :list="listCountry"
                   @update="select">
    </SelectCountry>
...
import SelectCountry from "@/views/user/list/component/Select";
import ScholarshipApi from "@/api/ScholarshipApi";
import Scholarship from "@/components/scholarship/Scholarship";

export default {
  name: '',
  components: {SelectCountry, Scholarship},
  data() {
    return {
      countryCurrentId: this.$route.query.id?this.$route.query.id:1,
      listCountry: [],
  ...
  methods: {
    ...
    select(id) {
      this.countryCurrentId = id;
      this.find(id);
    },
    async find(id) {
      try {
        let param = {countryId: id}
